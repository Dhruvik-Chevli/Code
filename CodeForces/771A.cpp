...
void dfs(vector<vector<ll> >&graph,vector<bool>&visited,ll start,ll &vert,ll &edge)
{
    visited[start]=true;
    vert++;
    edge+=graph[start].size();
    FOR(i,0,graph[start].size())
    {
        if(!(visited[graph[start][i]]))
        {
            dfs(graph,visited,graph[start][i],vert,edge);
        }
    }
    return;
}
...
int main()
{
    ...
    FOR(i,0,n)
    {
        if(!visited[i])
        {
            ll vert=0;
            ll edges=0;
            dfs(graph,visited,i,vert,edges);
            //cout<<vert<<" "<<edges;
            if(edges!=(vert)*(vert-1))
            {
                cout<<"NO\n";
                return 0;
            }
